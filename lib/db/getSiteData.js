...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  // ↓ 加在这里
  console.log('pageIds.length:', pageIds.length, 'collectionData.length:', collectionData.length, 'first value:', JSON.stringify(block[pageIds[0]]?.value?.properties))
  ...
  console.log('allPages.length:', allPages.length, 'sample type/status:', collectionData[0]?.type, collectionData[0]?.status)
  // Sort by date
...
function normalizeSchema(schema = {}) {
  ...
  Object.entries(schema).forEach(([key, value]) => {
    result[key] = { ...value, name: value?.name || '', type: value?.type || '' }
  })
  return result
}
