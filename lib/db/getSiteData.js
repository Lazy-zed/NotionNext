...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  if (!pageRecordMap) {
    ...
    // 返回完整的空数据结构，否则 categoryOptions/tagOptions 等字段会是 undefined，
    // 导致 getStaticPaths 里的 Object.keys(undefined) 直接中断整个构建。
  ...
  const rawCollection = pageRecordMap.collection?.[collectionId] || pageRecordMap.collection?.[idToUuid(collectionId)] || {}
  const collection = normalizeCollection(rawCollection)
  const collectionQuery = pageRecordMap.collection_query
  const collectionView = pageRecordMap.collection_view
  const adaptedBlock = adapterNotionBlockMap({ block: pageRecordMap.block || {}, collection: pageRecordMap.collection || {} })
...
function normalizePageBlock(blockItem) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if ((current.type === 'collection_view_page' || current.type === 'collection_view') && current.collection_id) return current
    if (current.type || current.properties) return current
    if (current.value) { current = current.value; continue }
    break
  }
  return null
