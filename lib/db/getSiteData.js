...
async function convertNotionToSiteDate(pageId, from, pageRecordMap) {
  ...
  const collectionId = idToUuid(rawMetadata?.collection_id)
  console.log('collectionId:', collectionId)
  console.log('collectionQuery keys:', Object.keys(pageRecordMap.collection_query || {}))
  const rawCollection =
...
function normalizePageBlock(blockItem) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (current.type === 'page' && current.properties) return current
    if (current.value) { current = current.value; continue }
    break
  }
  return null
